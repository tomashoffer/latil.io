    ...
    <div className="min-h-screen bg-gradient-to-br from-accent-50 via-white to-primary-50/30">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { num: "1", title: solution.step1, desc: solution.step1Desc, color: "bg-accent-600", icon: Bot },
                    { num: "2", title: solution.step2, desc: solution.step2Desc, color: "bg-primary-600", icon: Puzzle },
                    { num: "3", title: solution.step3, desc: solution.step3Desc, color: "bg-accent-600", icon: ShieldCheck },
                  ].map((step, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      <div
                        className={`w-7 h-7 ${step.color} text-white rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0`}
                      >
                        {step.num}
                      </div>
                      <div>
                ...
                <div className="bg-white rounded-2xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 border-b border-gray-100">
                    <div className="px-5 py-4 border-r border-gray-100">
                      <p className="text-xs text-gray-500 mb-1">{d.stat1Label}</p>
                      <p className="text-2xl font-bold text-gray-900">{d.stat1Value}</p>
                    </div>
                    <div className="px-5 py-4">
                      <p className="text-xs text-gray-500 mb-1">{d.stat2Label}</p>
                      <p className="text-2xl font-bold text-accent-600">{d.stat2Value}</p>
                    </div>
                  </div>

                  <div className="px-5 py-4 space-y-3">
                    <p className="text-xs font-medium text-gray-500">{d.agentsTitle}</p>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <MessageSquare size={14} className="text-gray-400" />
                          <span className="text-sm text-gray-700">{d.row1Label}</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-700">{d.row2Label}</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-700">{d.row3Label}</span>
                        </div>
                        <span className="text-xs font-medium text-primary-600 bg-primary-50 px-2 py-0.5 rounded-full">
                          {d.badge3}
                        </span>
            ...
            <div className="relative max-w-md mx-auto">
              <div className="bg-gradient-to-br from-accent-600 to-primary-600 rounded-2xl p-1 shadow-xl">
                <div className="bg-white rounded-xl overflow-hidden">
                  ...
                  <div className="px-4 py-4 border-b border-gray-100 bg-gradient-to-r from-accent-50 to-primary-50">
                    <p className="text-xs text-gray-500 mb-1">{d.mainLabel}</p>
                    <p className="text-2xl font-bold text-accent-600">{d.mainValue}</p>
                  </div>
                  <div className="grid grid-cols-2">
                    <div className="px-4 py-3 border-r border-gray-100">
                      <p className="text-xs text-gray-500">{d.stat1Label}</p>
                      <p className="text-xl font-bold text-gray-900">{d.stat1Value}</p>
                    </div>
                    <div className="px-4 py-3">
                      <p className="text-xs text-gray-500">{d.stat2Label}</p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-sm text-gray-500 mb-6 text-center">{solution.clients}</p>
          <div className="flex flex-wrap justify-center items-center gap-3 md:gap-4">
            {(solution.clientsList || []).map((org: string, idx: number) => (
              <div
                key={idx}
